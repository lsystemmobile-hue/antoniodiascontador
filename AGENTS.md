...
| [src/components/HeroSection.tsx](src/components/HeroSection.tsx) | Headline principal + CTA |
...
- `modelo_ia` — nome do modelo (ex: `gemini-1.5-flash`, `llama-3.3-70b-versatile`, `Codex-haiku-4-5-20251001`)
...
**Re-deploy após mudanças nas Edge Functions (via Management API):**
```python
# Atualizar chat
import urllib.request, json
TOKEN = os.environ['SUPABASE_MANAGEMENT_TOKEN']
with open('supabase/functions/chat/index.ts') as f: code = f.read()
req = urllib.request.Request(
  'https://api.supabase.com/v1/projects/qtxtvmgpxgnjtrrukggn/functions/chat',
  data=json.dumps({'body': code}).encode(),
  method='PATCH',
  headers={'Authorization': f'Bearer {TOKEN}', 'Content-Type': 'application/json', 'User-Agent': 'curl/7.88.1'}
)
